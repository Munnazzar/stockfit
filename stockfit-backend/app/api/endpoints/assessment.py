...
from app.schemas.assessment import QuestionnaireDetailSchema, QuestionnaireSchema, QuestionSchema, RiskAssessmentRequest, RiskAssessmentResponse
...
@router.get("/questionnaires/{questionnaire_id}", response_model=QuestionnaireDetailSchema)
def get_questionnaire_detail(
    questionnaire_id: str,
    db: psycopg2.extensions.connection = Depends(get_db),
    user_id: str = Depends(get_current_user_id),
) -> QuestionnaireDetailSchema:
    return assessment_service.get_questionnaire_detail(db, user_id, questionnaire_id)

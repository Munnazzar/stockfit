from .constants import LOW, MODERATE, HIGH, REQUIRED_RETURN_THRESHOLD, TIME_HORIZON_THRESHOLD, BEHAVIORAL_RISK_TIER_THRESHOLD
...
def solve_required_return_monthly(
    ...
    A = current_portfolio_value
    S = monthly_net_cash_flow
    N = investment_time_horizon_years * 12  # total months
    G = target_future_value
    ...
    def future_value(r):
        ...
        if abs(r) < 1e-10:
            return A + S * N
        return A * (1 + r) ** N + S * (((1 + r) ** N - 1) / r)

    low, high = -0.99 / 12, 10.0 / 12  # monthly bounds (~-99% to 100% annual)
...
def get_user_risk_need_tier(target_future_value: float, current_portfolio_value: float, investment_time_horizon_years: int, annual_net_cash_flow: float):
    ...
    r = solve_required_return_monthly(
    ...
    if r <= REQUIRED_RETURN_THRESHOLD[LOW]:
        tier = LOW
    elif r <= REQUIRED_RETURN_THRESHOLD[MODERATE]:
        tier = MODERATE
    else:
        tier = HIGH
